...
from data.Data_RealRain1K_L import Train_RealRain1K_L_Aug_Dataset, Test_RealRain1K_L_Dataset_whole_resolution
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('--train_batch_size',type=int,      default = [8,12,5,3,2,1])
    ...
    parser.add_argument('--model_dir',      type=str,       default='./training_model/RealRain1K_L_Baseline')
    parser.add_argument('--runs_dir',      type=str,       default='./runs/RealRain1K_L_Baseline')
...
class Session():
    ...
    def get_train_dataloader(self,epoch):
        if(epoch > 0 and epoch <= self.train_milestone[0]):
            dataset = Train_RealRain1K_L_Aug_Dataset(self.train_patch_size[0])
            ...
            dataset = Train_RealRain1K_L_Aug_Dataset(self.train_patch_size[1])
            ...
            dataset = Train_RealRain1K_L_Aug_Dataset(self.train_patch_size[2])
            ...
            dataset = Train_RealRain1K_L_Aug_Dataset(self.train_patch_size[3])
            ...
            dataset = Train_RealRain1K_L_Aug_Dataset(self.train_patch_size[4])
            dataloader = DataLoader(dataset, batch_size=self.train_batch_size[4], shuffle=True,num_workers=cfg.num_workers, drop_last=True)
        else:
            dataset = Train_RealRain1K_L_Aug_Dataset(self.train_patch_size[5])
...
def run_train_val():
    ...
    for epoch in range(sess.initial_epoch, sess.n_epochs+1):
        ...
        if epoch % cfg.val_freq == 0:           #5个epoch保存一次
            ...
            logfile = open('./log_test/' + 'val_RealRain1K_L_Baseline' + '.txt', 'a+')
            logfile.write(
                'epoch = ' + str(epoch) + '\t'
                 'ssim  = ' + str(ssim_val) + '\t'
                 'pnsr  = ' + str(psnr_val) + '\t'
